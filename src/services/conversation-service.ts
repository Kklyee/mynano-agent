...
export class ConversationService {
	...
	async updateConversationStatus(conversationId: string, status: PersistedConversationStatus): Promise<void> {
	...
	async getConversationDetail(input: GetConversationInput): Promise<ConversationDetail> {
		...
		const [messages, tools, tasks, taskEvents, backgroundTasks, backgroundEvents] = await Promise.all([
	...
	async prepareRun(input: PrepareRunInput): Promise<PreparedConversationRun> {
		...
		return {
			conversation,
			history: messages.filter((m) => m.role !== "tool").map((m) => ({ role: m.role, content: m.content })),
			nextSequence
		};
	}
	...
	async recordToolCallCompleted(input: RecordToolCallCompletedInput): Promise<void> {
		await this.repo.completeLatestToolCall(input.conversationId, input.name, input.result, new Date().toISOString());
	}
