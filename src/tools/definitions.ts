...
import { runSubagent } from "../agent/subagents/subagent";
import type { Task } from "../types";
import type { ToolContext, ToolDefinition } from "./context";
...
export const toolDefinitions: ToolDefinition[] = [
			...
			const subagentTools = ctx
				...
				.filter(
					(tool): tool is OpenAI.Chat.Completions.ChatCompletionFunctionTool =>
						tool.type === "function" && tool.function.name !== "delegate_to_subagent"
				);
			const result = await runSubagent({
				model: ctx.model,
				client: ctx.client,
				tools: subagentTools,
				runTool: (name: string, args: any) => executeTool(name as ToolsType, args, ctx),
				prompt: taskStr
			});
			return `子智能体结果:\n${result}`;
		}
	...
	},
	{
		...
		handler: async ({ subject, description, blockedBy }, { taskManager }) => {
			...
			const task = await taskManager.create(String(subject), String(description ?? ""), deps);
	...
	},
	{
		...
		handler: async ({ taskId, status, blockedBy, owner, description }, { taskManager }) => {
			if (!taskManager) {
				return "任务管理器未启用";
			}
			const updates: any = {};
	...
	},
	{
		...
		handler: async ({ taskId }, { backgroundManager }) => {
			...
			return task ? JSON.stringify(task, null, 2) : `Background task not found: ${taskId}`;
		}
	},
	{
		name: "background_list",
		description: "列出当前会话的所有后台任务",
		parameters: {
			type: "object",
			properties: {},
			required: []
		},
		handler: async (_args, { backgroundManager }) => {
			if (!backgroundManager) {
				return "后台任务管理器未启用";
			}
			...
			return tasks
				.map((t) => `[${t.id}] ${t.status.padEnd(9)} ${t.command}` + (t.pid ? ` (pid=${t.pid})` : ""))
				.join("\n");
		}
	},
	{
		...
		handler: async ({ taskId }, { backgroundManager }) => {
			...
			const cancelled = await backgroundManager.cancel(String(taskId));
			return cancelled ? `已取消后台任务: ${taskId}` : `取消失败：任务 ${taskId} 不存在或已结束`;
		}
	}
];

export function getToolDefinitions(enabledTools?: ToolsType[]): OpenAI.Chat.Completions.ChatCompletionTool[] {
...
export async function executeTool(name: ToolsType, args: unknown, ctx: ToolContext): Promise<string> {
