...
const DEFAULT_AUTH_SECRET =
  "mini-agent-better-auth-development-secret-please-change";
...
const authDbPath = process.env.AUTH_DB_PATH ?? join(process.cwd(), "data", "auth.sqlite");
...
export function ensureAuthSchema() {
  if (!schemaReadyPromise) {
    schemaReadyPromise = getMigrations(auth.options).then(({ runMigrations }) =>
      runMigrations(),
    );
  }
